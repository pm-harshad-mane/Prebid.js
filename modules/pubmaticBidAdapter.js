import { logWarn, _each, isStr, isBoolean, isNumber, isArray, inIframe,
  deepAccess, logError, deepClone, deepSetValue, convertTypes } from '../src/utils.js';
...
function _handleDealCustomTargetings(payload, dctrArr, validBidRequests) {
  ...
  if (dctrArr.length > 0) {
    if (validBidRequests[0].params.hasOwnProperty('dctr')) {
      ...
      if (dctrArr.length > 1) {
        logWarn(LOG_WARN_PREFIX + 'dctr value found in more than 1 adunits. Value from 1st adunit will be picked. Ignoring values from subsequent adunits');
      }
    } else {
      logWarn(LOG_WARN_PREFIX + 'dctr value not found in 1st adunit, ignoring values from subsequent adunits');
    }
  }
}
  ...
  isBidRequestValid: bid => {
    if (bid && bid.params) {
      ...
      if (bid.params.hasOwnProperty('video')) {
        if (!bid.params.video.hasOwnProperty('mimes') || !isArray(bid.params.video.mimes) || bid.params.video.mimes.length === 0) {
          logWarn(LOG_WARN_PREFIX + 'Error: For video ads, mimes is mandatory and must specify atlease 1 mime value. Call to OpenBid will not be sent for ad unit:' + JSON.stringify(bid));
          return false;
        }
        if (bid.hasOwnProperty('mediaTypes') && bid.mediaTypes.hasOwnProperty(VIDEO)) {
          ...
          if (bid.mediaTypes[VIDEO].context === 'outstream' && !isStr(bid.params.outstreamAU)) {
            logError(`${LOG_WARN_PREFIX}: for "outstream" bids outstreamAU is required. Rejecting bid: `, bid);
            return false;
          }
        } else {
          logError(`${LOG_WARN_PREFIX}: mediaTypes or mediaTypes.video is not specified. Rejecting bid: `, bid);
          return false;
        }
      }
      return true;
